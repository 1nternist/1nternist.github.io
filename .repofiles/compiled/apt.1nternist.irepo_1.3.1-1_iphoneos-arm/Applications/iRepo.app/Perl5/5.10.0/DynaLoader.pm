...
#         = @Config::Config{'dlext', 'dlsrc'};
  ($dl_dlext, $dlsrc) = ('dylib', 'dl_dlopen.xs')
;
...
sub dl_findfile {
    ...
    #my $dl_ext= 'dylib'; # $Config::Config{'dlext'} suffix for perl extensions
    ...
    arg: foreach(@args) {
        ...
        if (m:-l: ) {          # convert -lname to appropriate library name
            s/-l//;
            push(@names,"lib$_.dylib");
            push(@names,"lib$_.a");
        } else {                # Umm, a bare name. Try various alternatives:
            # these should be ordered with the most likely first
            push(@names,"$_.dylib")    unless m/\.dylib$/o;
            push(@names,"$_.dylib")     unless m/\.dylib$/o;
            push(@names,"lib$_.dylib")  unless m:/:;
